...
export default async function Page({
  ...
  const promos = await (prisma as any).promoPlacement.findMany({
    include: { offer: { include: { operator: true } } },
    orderBy: [{ slot: 'asc' }, { weight: 'asc' }],
  });

  return (
    <Guard keyParam={keyParam}>
      <main className="container mx-auto px-4 py-8">
        <h1 className="text-2xl font-bold">Admin — Promos</h1>
            ...
            {promos.map((p: any) => (
              <div key={p.id} className="rounded border p-3">
                ...
                <div className="font-semibold">
                  {p.offer.operator.name} — {p.offer.title}
                </div>
                <form action={updatePromo} className="mt-2 grid gap-2 md:grid-cols-2">
                  ...
                  <input
                    className="rounded border px-3 py-2"
                    name="label"
                    defaultValue={p.label ?? 'Sponsored'}
                  />
                  <input
                    className="rounded border px-3 py-2"
                    name="weight"
                  ...
                  />
                  <label className="flex items-center gap-2 text-sm">
                    <input type="checkbox" name="isActive" defaultChecked={p.isActive} /> Activ
                  </label>
                  <input
                    className="rounded border px-3 py-2"
                    type="datetime-local"
                    name="startAt"
                    defaultValue={p.startAt ? new Date(p.startAt).toISOString().slice(0, 16) : ''}
                  />
                  <input
                    className="rounded border px-3 py-2"
                    type="datetime-local"
                    name="endAt"
                    defaultValue={p.endAt ? new Date(p.endAt).toISOString().slice(0, 16) : ''}
                  />
