  ...
  return (
    <Guard keyParam={keyParam}>
      <main className="container mx-auto px-4 py-8">
        <h1 className="text-2xl font-bold">Admin ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œ Ad Spend</h1>
        <form action={createSpend} className="mt-4 grid gap-2 md:grid-cols-4">
          ...
          <input className="rounded border px-3 py-2" name="source" placeholder="sursÃƒÆ’Ã¢â‚¬Å¾Ãƒâ€ Ã¢â‚¬â„¢ (ex: google)" required />
          <input className="rounded border px-3 py-2" name="campaign" placeholder="campanie (opÃƒÆ’Ã‹â€ ÃƒÂ¢Ã¢â€šÂ¬Ã‚Âºional)" />
          <input className="rounded border px-3 py-2" type="number" step="0.01" name="amount" placeholder="RON" />
          <button className="rounded border px-3 py-2 font-semibold md:col-span-4" type="submit">AdaugÃƒÆ’Ã¢â‚¬Å¾Ãƒâ€ Ã¢â‚¬â„¢</button>
        </form>
        <table className="mt-6 w-full text-sm">
          <thead><tr><th className="p-2 text-left">Zi</th><th className="p-2">SursÃƒÆ’Ã¢â‚¬Å¾Ãƒâ€ Ã¢â‚¬â„¢</th><th className="p-2">Campanie</th><th className="p-2 text-right">SumÃƒÆ’Ã¢â‚¬Å¾Ãƒâ€ Ã¢â‚¬â„¢</th></tr></thead>
          <tbody>
            {latest.map((r: any) => (
              <tr key={r.id} className="border-t">
                <td className="p-2">{new Date(r.day).toLocaleDateString("ro-RO")}</td>
                <td className="p-2">{r.source}</td>
                <td className="p-2">{r.campaign ?? "ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â"}</td>
