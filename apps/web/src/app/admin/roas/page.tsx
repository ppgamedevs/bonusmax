  ...
  return (
    <Guard keyParam={keyParam}>
      <main className="container mx-auto px-4 py-8">
        <h1 className="text-2xl font-bold">Admin – ROAS / EPC</h1>
        ...
        <form className="mt-2 flex flex-wrap items-end gap-2">
          ...
          <button className="rounded border px-3 py-1 text-sm" type="submit">Aplică</button>
        </form>

        <section className="mt-6">
          <h2 className="text-lg font-semibold">Pe oferte</h2>
          <table className="mt-2 w-full text-sm">
            <thead><tr><th className="p-2 text-left">Operator</th><th className="p-2 text-left">Ofertă</th><th className="p-2 text-center">Clicks</th><th className="p-2 text-right">Revenue</th><th className="p-2 text-right">EPC</th></tr></thead>
            <tbody>
              {byOffer.map((r: any) => (
        ...
        <section className="mt-8">
          ...
          <table className="mt-2 w-full text-sm">
            <thead><tr><th className="p-2 text-left">Sursă</th><th className="p-2 text-right">Spend</th><th className="p-2 text-right">Revenue</th><th className="p-2 text-right">ROAS</th></tr></thead>
            <tbody>
              {spend.map((s: any) => (
          ...
          <p className="mt-2 text-xs opacity-70">Notă: pentru ROAS complet pe surse, extindeți atribuirea cu revenue sum pe UTM source (join RevenueEvent → ClickEvent prin clickId).</p>
        </section>

        <p className="mt-6 text-xs opacity-60">CTR site-wide: {(ctr * 100).toFixed(2)}% • EPC/ROAS în RON</p>
      </main>
    </Guard>
  );
}
