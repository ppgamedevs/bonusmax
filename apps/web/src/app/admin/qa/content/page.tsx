...
export default async function Page({ searchParams }: { searchParams?: Promise<Record<string, string>> }) {
  ...
  const sev = searchParams?.sev as ("ERROR"|"WARN"|"INFO"|undefined);
  const type = searchParams?.type as ("OFFER"|"GUIDE"|"OPERATOR"|undefined);
  ...
  return (
    <Guard keyParam={keyParam}>
      <main className="container mx-auto px-4 py-8">
        <h1 className="text-2xl font-bold">Admin ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Content QA</h1>
          ...
          <tbody>
            {issues.map((i: any) => (
              <tr key={i.id} className="border-t">
                ...
                <td className="p-2">{i.entityType} Ãƒâ€šÃ‚Â· {i.entityId}</td>
        ...
        <p className="mt-4 text-xs opacity-60">
          Reguli: FWD-001/002 (formulÃƒâ€žÃ†â€™ri interzise), WR-001, DEP-001, DATE-001, LIC-001, SEO-001/002, DISC-001, H1-001.
        </p>
      </main>
    </Guard>
  );
