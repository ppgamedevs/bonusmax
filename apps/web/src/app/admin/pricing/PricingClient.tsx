  ...
  return (
    <main className="container mx-auto px-4 py-8">
      ...
      <p className="mt-2 text-sm opacity-80">
        Baseline 30z: Impr {base.impressions.toLocaleString('ro-RO')} / Clicks{' '}
        {base.clicks.toLocaleString('ro-RO')} / CTR {(base.ctr * 100).toFixed(1)}%
      </p>
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-3">
        <label className="rounded border p-3 text-sm">
          ...
          <input
            ...
            type="number"
            value={imps}
            onChange={(e) => setImps(Number(e.target.value || 0))}
          />
        </label>
        <label className="rounded border p-3 text-sm">
          CTR estimat (%)
          <input
            ...
            onChange={(e) => setCtr(Number(e.target.value) / 100)}
          />
        </label>
      ...
      <p className="mt-6 text-xs opacity-60">
        Note: CTR variază în funcție de poziționare și creativ. Folosește valorile din 30 de zile ca
        punct de plecare.
      </p>
    </main>
  );
