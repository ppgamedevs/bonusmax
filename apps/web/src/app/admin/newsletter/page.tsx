...
async function createIssue(formData: FormData) {
  ...
  const subject = String(formData.get("subject") || "Noutăți Bonusmax — săptămâna aceasta");
  ...
  <div style="font-family:Arial,sans-serif">
    <h1>${subject}</h1>
    <p style="font-size:12px;opacity:.7">18+ Joacă responsabil • Conținut comercial marcat</p>
    <ul>
      ${items
        .map(
          (i: any) =>
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        <form action={createIssue} className="mt-3 flex flex-wrap items-end gap-2">
          ...
          <label className="text-sm">
            Subiect
            <input name="subject" className="ml-2 rounded border px-2 py-1" defaultValue="Noutăți Bonusmax — săptămâna aceasta" />
          </label>
          <label className="text-sm">
            Item IDs (virgule)
            ...
            <tbody>
              {(last7 as any[]).map((i: any) => (
                <tr key={i.id} className="border-t">
                  ...
                  <td className="p-2">{i.publishedAt ? new Date(i.publishedAt).toLocaleDateString("ro-RO") : "—"}</td>
