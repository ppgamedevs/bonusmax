...
function Guard({ children, keyParam }: { children: any; keyParam?: string }) {
  if (!process.env.ADMIN_KEY || keyParam !== process.env.ADMIN_KEY)
    return <main className="p-8">401</main>;
  return <>{children}</>;
}
...
async function createIssue(formData: FormData) {
  ...
  const subject = String(formData.get('subject') || 'Noutăți Bonusmax');
  ...
  <div style="font-family:Arial,sans-serif">
    <h1>${subject}</h1>
    <p style="font-size:12px;opacity:.7">18+ Joacă responsabil • Conținut comercial</p>
    <ul>
      ${items
        .map(
          (i: any) =>
            `<li><a href="${i.url}" target="_blank" rel="nofollow noopener">${i.title}</a> — <span style="opacity:.7">${i.excerpt || ''}</span></li>`
        )
        .join('')}
    </ul>
    <p style="font-size:12px;opacity:.7">Te poți dezabona oricând.</p>
  </div>`;
  const issue = await (prisma as any).newsletterIssue.create({
    data: { number: num, subject, html },
  });
...
export default async function Page({
  ...
  const issues = await (prisma as any).newsletterIssue.findMany({
  ...
  return (
    <Guard keyParam={key}>
      <main className="container mx-auto px-4 py-8">
        <h1 className="text-2xl font-bold">Newsletter — Compose</h1>
        <form action={createIssue} className="mt-3 flex flex-wrap items-end gap-2">
          ...
          <label className="text-sm">
            ...
            <input
              name="ids"
              className="ml-2 w-[420px] rounded border px-2 py-1"
              placeholder={(last7 as any[])
                .map((i: any) => i.id)
                .slice(0, 6)
                .join(',')}
            />
          </label>
          <button className="rounded border px-3 py-1">Generează</button>
        </form>
            ...
            <tbody>
              {(last7 as any[]).map((i: any) => (
                <tr key={i.id} className="border-t">
                  ...
                  <td className="p-2">
                    {i.publishedAt ? new Date(i.publishedAt).toLocaleDateString('ro-RO') : '—'}
                  </td>
          ...
          <ul className="mt-2 list-disc pl-5 text-sm">
            {(issues as any[]).map((i: any) => (
              <li key={i.id}>
                <a
                  className="underline"
                  href={`/api/admin/newsletter/issues/${i.id}/export?key=${key}`}
                  target="_blank"
                >
                  Export HTML
                </a>{' '}
