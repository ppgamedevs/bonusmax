...
// Simple in-memory rate limit (per process). For production, consider Redis.
const buckets = new Map<string, { t: number; c: number }>();
function rateOk(key: string, limit = 20, windowMs = 10 * 60 * 1000) {
  const now = Date.now();
...
export async function POST(req: Request) {
  ...
  // rate limit
  const ipHash = hashIp(String(ip), ua);
  if (!rateOk(`partners:lead:${ipHash}`))
  ...
  const score = scoreLead({
    ...
    onjnLicenseId: body.onjnLicenseId,
  });
