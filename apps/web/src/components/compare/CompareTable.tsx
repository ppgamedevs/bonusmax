...
export default function CompareTable({
  ...
  if (!offers?.length) {
    return <p className="text-sm opacity-80">Nu ai ales ÃƒÂ®ncÃ„Æ’ oferte pentru comparaÃˆâ€ºie.</p>;
  }
      ...
      <table className="w-full border-separate border-spacing-0 text-sm">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-2 text-[11px]">
                          {o.isLicensedRO ? (
                            ...
                            <span className="opacity-70">Ã¢â‚¬â€</span>
                          )}
                          {o.isSponsored && (
                      ...
                      {win && (
                        <span
                          ...
                          aria-label="CÃƒÂ¢Ãˆâ„¢tigÃ„Æ’tor EPC (30 zile)"
                          title="CÃƒÂ¢Ãˆâ„¢tigÃ„Æ’tor EPC (30 zile)"
                        >
                          Ã¢Â­Â {badgeLabel}
                        </span>
                      )}
        ...
        <tbody>
            ...
            {offers.map((o) => {
              ...
              return (
                <td key={o.id} className={colClass}>
                  <div className={colWrap(win)}>
                    <div className="font-semibold">{o.headline || "Ã¢â‚¬â€"}</div>
          ...
          {[ 
            ...
            { label: "Depunere minimÃ„Æ’", key: "minDeposit" as const },
            { label: "Rotiri gratuite", key: "spins" as const },
            { label: "Timp limitÃ„Æ’", key: "timeLimit" as const },
          ...
          <tr className="border-t border-white/10 even:bg-white/5 dark:even:bg-white/5">
            <td className="p-3 text-xs opacity-70">AplicaÃˆâ€ºie mobilÃ„Æ’</td>
            {offers.map((o) => {
              const win = o.id === highlightOfferId;
              return (
                <td key={o.id} className={colClass}>
          ...
          <tr className="border-t border-white/10 even:bg-white/5 dark:even:bg-white/5">
            <td className="p-3 text-xs opacity-70">PlÃ„Æ’Ãˆâ€ºi</td>
            {offers.map((o) => {
              const win = o.id === highlightOfferId;
              return (
                <td key={o.id} className={colClass}>
                  <div className={colWrap(win)}>
                    {o.payments?.length ? (
                      <div className="text-xs opacity-80">{o.payments.join(" Ã¢â‚¬Â¢ ")}</div>
                    ) : (
                      no
                    )}
                  </div>
                ...
                <td key={o.id} className={colClass}>
                  <div className={colWrap(win)}>
                    <Link
                      ...
                      aria-label={`RevendicÃ„Æ’ bonusul la ${o.brand}`}
                    >
                      RevendicÃ„Æ’ bonusul
                    </Link>
