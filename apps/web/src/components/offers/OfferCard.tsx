...
export default function OfferCard({
  ...
  function WRTooltip() {
    const tip =
      'WR (Wagering Requirement): rulajul necesar pentru a transforma bonusul în bani reali.';
    return (
      <span className="relative inline-block">
        <span className="underline decoration-dotted cursor-help" aria-label={tip} title={tip}>
          WR
        </span>
      </span>
    );
    ...
    >
      ...
      <div className="mt-3 space-y-2">
        ...
        <div className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-neutral-500 dark:text-neutral-300">
          ...
          {(minDeposit ?? null) && (
            <span>
              💳 Min dep: {typeof minDeposit === 'number' ? `${minDeposit} RON` : minDeposit}
            </span>
          )}
