...
export default function HomeCompareSectionClient() {
  ...
  const bestId = useMemo(
    () => (offers && offers.length ? heuristicBestValue(offers) : undefined),
    [offers]
  );
    ...
    <section className="mx-auto mt-10 max-w-6xl px-4">
      <header className="mb-3">
        ...
        <p className="mt-1 text-sm opacity-70">
          Alege 2–3 oferte și compară-le rapid. Caută „Compară” pe carduri pentru a selecta.
        </p>
      </header>
        ...
        <>
            ...
            <table className="w-full border-separate border-spacing-0 text-sm">
                  ...
                  {offers.map((o) => (
                    <th key={o.id} className="min-w-[220px] align-top p-3">
                      <div
                        className={
                          (o.id === bestId
                            ? 'ring-1 ring-violet-300/40 bg-violet-500/[0.05] '
                            : '') + 'rounded-lg p-2'
                        }
                      >
                        <div className="flex items-center gap-2">
                          ...
                          {o.id === bestId && (
                            <span className="ml-auto rounded-full bg-violet-500/15 px-2 py-[2px] text-[10px] font-medium text-violet-600 dark:text-violet-400">
                              Best Value
                            </span>
                          )}
                        </div>
              ...
              <tbody>
                {[
                  ...
                  {
                    label: 'WR',
                    val: (o: any) => o.wr || (o.wrMultiplier ? `x${o.wrMultiplier}` : '—'),
                  },
                  {
                    label: 'Depunere minimă',
                    val: (o: any) =>
                      typeof o.minDeposit === 'number'
                        ? `${o.minDeposit} RON`
                        : o.minDeposit || '—',
                  },
                  ...
                  {
                    label: 'Valabilitate',
                    val: (o: any) => o.validity || (o.validDays ? `${o.validDays} zile` : '—'),
                  },
                ].map((row, idx) => (
                  <tr
                    key={row.label}
                    className={
                      (idx % 2 === 0
                        ...
                        : 'bg-white/10 dark:bg-black/10') + ' border-t border-white/10'
                    }
                  >
            ...
            <div className="flex snap-x snap-mandatory gap-3 pb-2">
              {offers.map((o) => (
                <div
                  key={o.id}
                  className={
                    (o.id === bestId ? 'ring-1 ring-violet-300/40 bg-violet-500/[0.05] ' : '') +
                    'snap-start shrink-0 basis-[90%] rounded-2xl border border-white/10 p-4'
                  }
                >
                  <div className="flex items-center gap-2">
                    <div className="grid h-10 w-10 place-items-center overflow-hidden rounded-lg bg-black/10 ring-1 ring-black/10 dark:bg-white/10 dark:ring-white/10">
                  ...
                  <ul className="mt-2 space-y-1 text-sm">
                    <li>
                      <span className="opacity-70">Max bonus:</span>{' '}
                      {o.maxBonus || o.headline || '—'}
                    </li>
                    <li>
                      <span className="opacity-70">WR:</span>{' '}
                      {o.wr || (o.wrMultiplier ? `x${o.wrMultiplier}` : '—')}
                    </li>
                    <li>
                      <span className="opacity-70">Min dep:</span>{' '}
                      {typeof o.minDeposit === 'number'
                        ? `${o.minDeposit} RON`
                        : o.minDeposit || '—'}
                    </li>
                    <li>
                      <span className="opacity-70">Rotiri:</span> {o.spins || o.freeSpins || '—'}
                    </li>
                    <li>
                      <span className="opacity-70">Valabilitate:</span>{' '}
                      {o.validity || (o.validDays ? `${o.validDays} zile` : '—')}
                    </li>
                  </ul>
                  <Link
                    href={`/go/${o.id}` as any}
          ...
          <div className="mt-4 flex justify-center">
            <Link href={'/compara' as any} className="btn-accent h-11 px-6 focus-accent">
              Alege bonusul și revendică acum →
            </Link>
          </div>
        </>
      )}
