...
function RippleButton(props: React.ComponentProps<typeof Link>) {
  ...
  return (
    <Link
      ...
      className={(props.className || '') + ' btn-accent focus-accent relative overflow-hidden'}
    />
  );
}
    ...
    >
      <div className="relative overflow-hidden rounded-3xl border border-white/15 bg-neutral-950/80 p-5 shadow-xl ring-1 ring-white/10 dark:bg-neutral-900/70" style={{ containIntrinsicSize: '380px', contentVisibility: 'auto' as any }}>
        ...
        <div className="relative z-10 mt-4 flex items-center gap-3">
          <RippleButton
            href={ctaHref}
            aria-label={`Revendică bonusul ${brand}`}
            className="btn-accent h-12 px-6 text-base"
          >
            Revendică bonusul
          </RippleButton>
      ...
      <div className="mt-1 text-[11px] opacity-70">
        Exemplu ilustrativ pentru prezentare • 18+ Joacă responsabil • Publicitate
      </div>
    </motion.section>
  );
}
