...
export async function sendConfirmEmail(to: string, rawToken: string, subscriberId: string) {
  ...
  await sendEmail({
    to,
    subject: 'Confirmă abonarea la alertele Bonusmax',
    html,
    text: plain(html),
  });
}
...
export async function sendWeeklyDigest(
  to: string,
  subscriberId: string,
  offers: Array<{ id: string; title: string; operator: { name: string } }>
) {
  const items = offers
    .map(
      (o) =>
        `<li><a href="${absoluteUrl(`/e/click?sid=${subscriberId}&oid=${o.id}`)}" rel="nofollow" style="text-decoration:none">${o.operator.name} — ${o.title}</a></li>`
    )
    .join('');
  const htmlBody = `
    ...
    <img src="${absoluteUrl(`/i/email/open?sid=${subscriberId}`)}" width="1" height="1" alt="" />
  `;
  const html = emailLayout('Top bonusuri — Săptămâna aceasta', htmlBody).replace(
    '__UNSUB__',
    '__UNSUB__'
  );
