...
export type Experiment = { key: "CTA_COPY_V1" | "OFFERS_ORDER_V1" | "STICKY_BAR_V1" | "EXIT_INTENT_V1"; status: "ACTIVE" | "PAUSED"; paths: string[]; variants: Variant[] };
...
function matchesPath(paths: string[], pathname: string) {
  return paths.some((p) => p === pathname || (p.endsWith("*") && pathname.startsWith(p.slice(0, -1))));
}
...
export function getVariant(expKey: Experiment["key"]): "A" | "B" {
  ...
  if (!val) {
    ...
    cAny?.set?.(cookieName, val, { path: "/", maxAge: 60 * 60 * 24 * 30, httpOnly: false, sameSite: "lax" });
  }
  return val === "B" ? "B" : "A";
}
