...
export async function buildPreview(rows: Row[]): Promise<PreviewItem[]> {
  ...
  for (const r of rows) {
    ...
    for (const f of fields) {
      ...
      const same =
        f === "onjnLicenseExpiry"
          ? (oldVal ? new Date(oldVal).getTime() : null) === (newVal ? new Date(newVal).getTime() : null)
...
export async function commitPreview(items: PreviewItem[]) {
  const creates = items.filter((i) => i.kind === "CREATE") as Extract<PreviewItem, { kind: "CREATE" }>[];
  const updates = items.filter((i) => i.kind === "UPDATE") as Extract<PreviewItem, { kind: "UPDATE" }>[];
