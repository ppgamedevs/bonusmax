...
export type Issue = { ruleId: string; severity: Severity; message: string; meta?: unknown };
...
function findQaFile(file: string): string {
  ...
  for (const p of candidates) {
    try {
      ...
      // Intentionally ignore fs access errors for candidate paths; we'll fall back to default.
    }
  }
...
const offerRules: Rule<OfferDTO>[] = [
        ...
        ? [
            {
              ruleId: 'FWD-001',
              severity: 'ERROR',
              message: `Formulări interzise: ${hits.join(', ')}`,
            },
          ]
        : [];
    },
        ...
        return [
          {
            ruleId: 'WR-001',
            severity: 'ERROR',
            message: `WR ${wr}x pare excesiv (>70x). Verifică sursa.`,
          },
        ];
      if (wr > 50)
        return [{ ruleId: 'WR-001', severity: 'WARN', message: `WR ${wr}x ridicat (>50x).` }];
      if (wr < 1)
        return [{ ruleId: 'WR-001', severity: 'WARN', message: `WR ${wr}x prea mic (<1x).` }];
      return [];
    },
  },
  {
    ...
    apply(o) {
      ...
      if (md != null && md < 0)
        issues.push({
          ruleId: 'DEP-001',
          severity: 'ERROR',
          message: `MinDeposit negativ (${md}).`,
        });
      if (o.offerType === 'FARA_DEPUNERE' && md && md > 0)
        issues.push({
          ruleId: 'DEP-001',
          severity: 'WARN',
          message: `Bonus fără depunere dar minDeposit=${md}.`,
        });
      return issues;
    },
  },
  {
    ...
    apply(o) {
      ...
      if (startAt && endAt && endAt < startAt)
        issues.push({ ruleId: 'DATE-001', severity: 'ERROR', message: 'endAt < startAt' });
      const now = new Date();
      if (isActive && endAt && endAt < now)
        issues.push({
          ruleId: 'DATE-001',
          severity: 'WARN',
          message: 'Oferta e expirată dar încă marcată activă.',
        });
      return issues;
    },
  },
        ...
        return [
          {
            ruleId: 'LIC-001',
            severity: 'ERROR',
            message: `Operatorul ${o.operator.name} nu este licențiat RO.`,
          },
        ];
      }
      return [];
  ...
  },
  {
    ...
    apply(o) {
      ...
      if (!between(L, 35, 66)) {
        return [
          { ruleId: 'SEO-001', severity: 'WARN', message: `Titlu ${L} caractere (ideal 35–66).` },
        ];
      }
      return [];
    },
  },
  {
    ...
    apply(o) {
      ...
      if (L < 20)
        return [{ ruleId: 'SEO-002', severity: 'WARN', message: 'termsShort prea scurt (<20).' }];
      if (L > 220)
        return [{ ruleId: 'SEO-002', severity: 'WARN', message: 'termsShort prea lung (>220).' }];
      return [];
    },
  },
];
...
const guideRules: Rule<GuideDTO>[] = [
        ...
        ? [
            {
              ruleId: 'FWD-002',
              severity: 'ERROR',
              message: `Formulări interzise în ghid: ${hits.join(', ')}`,
            },
          ]
        : [];
    },
  },
  {
    ...
    apply(g) {
      const ok =
        mdxHas(g.source, /18\+\s*Joac[ăa] responsabil/i) &&
        mdxHas(g.source, /Con[țt]inut comercial|Publicitate/i);
      return ok
        ? []
        : [
            {
              ...
              severity: 'WARN',
              message:
                'Adaugă disclaimere: „18+ Joacă responsabil” și „Conținut comercial/Publicitate”.',
            },
          ];
    },
  },
        ...
        : [
            {
              ruleId: 'H1-001',
              severity: 'WARN',
              message: 'Lip­sește H1 (linie ce începe cu `# `).',
            },
          ];
    },
  },
        ...
        return [
          {
            ruleId: 'SEO-101',
            severity: 'WARN',
            message: `Titlu ghid ${L} caractere (ideal 35–66).`,
          },
        ];
      return [];
    },
...
export async function reconcileIssues(
  ...
  const open = await prisma.contentQaIssue.findMany({
  ...
  if (toResolve.length) {
    await prisma.contentQaIssue.updateMany({
  ...
  return {
    ...
    added: toAdd.length,
    unchanged: newIssues.length - toAdd.length,
  };
}
